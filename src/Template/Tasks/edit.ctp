...
<nav class="col-lg-2 col-md-3 columns" id="actions-sidebar">
    <ul class="side-nav nav flex-column">
        ...
        <li class="nav-item"><?= $this->Form->postLink(
                ...
                ['class' => 'nav-link', 'confirm' => __('Are you sure you want to delete # {0}?', $task->id)]
            )
        ?></li>
        <li><?= $this->Html->link(__('List Tasks'), ['action' => 'index'], ['class' => 'nav-link']) ?></li>
    </ul>
</nav>
<div class="tasks form col-lg-3 col-md-3 columns content">
    ...
    <fieldset>
        ...
        <?php
            ...
            echo $this->Form->text('date', ['type' => 'date', 'id' => 'date', 'value' => isset($task->date) ? $task->date->format('Y-m-d') : "", 'class' => 'form-control form-control-sm mb-3']);
